...
def sentences2index(pairs, path2save):
    """
    ...
    file.create_dataset('fra_index_list', data=fra_index_list)
    file.create_dataset('eng_index_list', data=eng_index_list)
    file.create_dataset('fra_lengths_list', data=fra_lengths_list)
    file.create_dataset('eng_lengths_list', data=eng_lengths_list)
    file.close()
...
if __name__ == '__main__':
    ...
    #保存词典
    inputLang.save('../dataAfterProcess/fra_input_dict_%s.pkl'%(suffix))
    outputLang.save('../dataAfterProcess/eng_output_dict_%s.pkl'%(suffix))
    

    sentences2index(pairs, '../dataAfterProcess/dataset_fra2eng_%s.h5py'%(suffix))
